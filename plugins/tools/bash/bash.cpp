...
bool extractString(const std::string &json, const std::string &key,
  ...
  while (pos < json.size()) {
    ...
    if (!appendJsonEscape(value, escape))
      return false;
  }
...
std::string errnoMessage(const char *operation) {
  return std::string(operation) +
         " failed in bash tool: " + std::strerror(errno);
}
...
ExecResult execCommand(const std::string &command) {
  ...
  if (pipe(pipe_fds) == -1) {
    return ExecResult{
        .output = "", .error = errnoMessage("pipe"), .exit_code = -1};
  }
    ...
    return {
        {"bash",
         ...
         [](const std::string &input) -> std::string {
           ...
           if (command.empty())
             return R"({"error":"command is empty"})";
...
AFS_PLUGIN_EXPORT AFS::Plugin *createPlugin() { return new BashPlugin(); }
