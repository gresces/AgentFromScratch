...
| 文件 | 职责 |
|------|------|
| `model.hh` | 模型配置结构、`AFS_Model` 基类、OpenAI 兼容模型、DeepSeek 模型、工厂函数 |
| `model.cc` | HTTP 请求、JSON 解析、模型配置 schema 注册/解析、工厂函数 |
...
`AFS_ModelConfig` 与 `AFS_ModelsConfig` 属于 models 模块，不在 `basic/config/` 中声明。

| 类型 | 说明 |
|------|------|
| `AFS_ModelConfig` | 单个模型配置：`name`、`base_url`、`api_key`、`model`、`context_limit` |
| `AFS_ModelsConfig` | 模型分组：`llms` 和 `embeddings` |

| 函数 | 说明 |
|------|------|
| `AFS_RegisterModelConfigSchemas()` | 向 `AFS_ConfigManager` 注册 `models.llms` 和 `models.embeddings` schema |
| `AFS_LoadModelsConfig(config)` | 从 `AFS_Config` 解析 typed 模型配置；也保留 `AFS_ConfigManager` 便捷重载 |
| `AFS_ModelConfig::configSchema(path, module)` | 生成模型配置字段 schema |
...
| `static configSchema(path, module)` | `AFS_ConfigSchema` | 声明模型配置项结构；内建用于 `models.llms` 和 `models.embeddings` 注册 |
...
for (const auto& llm_cfg : models->llms) {
...
- `basic/config/config.hh` — `AFS_ConfigManager` 与 schema 注册类型。
